...
/**
  * La función crea un usuario inactivo basado en los datos proporcionados en la solicitud de registro en Java.
  * 
  * @param request El método `crearUsuarioInactivo` toma un objeto `SignupRequest` como parámetro.
  * Este objeto probablemente contiene la información necesaria para crear un nuevo usuario, como nombre, fecha de nacimiento,
  * correo electrónico, contraseña y tipo de usuario.
  * @return El método `crearUsuarioInactivo` devuelve un objeto `Usuario` que ha sido creado y
 ...
 /**
  * The `activarUsuario` function activates a user by setting their state to active in the database.
  * 
  * @param correoUsuario The method `activarUsuario` takes a `String` parameter `correoUsuario`, which
  * represents the email address of the user whose account needs to be activated. The method retrieves
  * the user from the repository based on the provided email address, sets the user's state to `ACTIVO`
  * (active),
  */
